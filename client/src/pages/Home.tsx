...
export default function Home() {
  ...
  // ✅ speak 원복: ROLES(역할 확인/넘기기)에서만 무음, 나머지는 정상
  const speak = (text: string) => {
    if (!ttsOn) return;
    if (typeof window === "undefined") return;

    // ✅ 역할 확인/넘기기 구간은 음성 지원 OFF
  ...
  const checkWinAndMaybeEnd = (aliveState: Record<string, boolean>) => {
    ...
    if (mafiaAliveCount === 0) {
      ...
      speak("모든 마피아가 제거되었습니다. 시민이 승리하였습니다. 축하합니다!");
      return true;
    }
    ...
    if (nonMafiaAliveCount === 0 && mafiaAliveCount > 0) {
      ...
      speak("모든 시민이 제거되었습니다. 마피아가 승리하였습니다. 축하합니다!");
      return true;
    }
  ...
  const goNightHandoff = (turn: "MAFIA" | "DOCTOR" | "POLICE") => {
    ...
    // ✅ 밤 시작부터 음성 시작(ROLES만 무음이므로 여기서부터 자연스럽게 나온다)
    if (turn === "MAFIA") {
      speak(
  ...
  const setPreset = (m: "Day" | "Night") => {
    ...
    speak(m === "Day" ? "낮 토론 타이머를 설정했습니다." : "밤 타이머를 설정했습니다.");
  };
  ...
  const executeVote = () => {
    ...
    if (!topVoted) {
      ...
      speak("집행할 수 없습니다. 투표가 없거나 동점입니다.");
      return;
    }
    ...
    <div className="min-h-[100svh] text-white bg-[#07070a] relative overflow-hidden">
      ...
      <div className="max-w-md mx-auto px-5 relative min-h-[100svh] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto pb-8">
          ...
          {phase === "RESULT" && (
            <div className={"mt-6 " + glowCard + " p-4"}>
              {/* (이하 UI는 너가 붙여준 그대로 유지) */}
              {/* ... */}
            </div>
          )}

          {/* (이하 NIGHT_* UI도 너가 붙여준 그대로 유지) */}
          {/* ... */}
